...
export const getAddressInfo = async (address, blockchain = 'ethereum') => {
    ...
    try {
        const response = await axios.get(`${BLOCKCHAIR_API}/${blockchain}/dashboards/address/${address}?limit=50`);
...
export const getRecentTransactions = async (address, blockchain = 'ethereum') => {
    ...
    try {
        const response = await axios.get(`${BLOCKCHAIR_API}/${blockchain}/transactions?address=${address}&limit=50&order=desc`);
        const data = response.data;
...
export const analyzeSuspiciousPatterns = (addressData, transactions) => {
    const findings = [];
    let riskScore = 0;
    ...
    const zeroValueTxs = transactions.filter(tx => parseFloat(tx.value) === 0 || tx.value === '0').length;
    if (zeroValueTxs > 10) {
    ...
    const largeTxs = transactions.filter(tx => parseFloat(tx.value) > 100 // > 100 ETH
    ).length;
    if (largeTxs > 0) {
